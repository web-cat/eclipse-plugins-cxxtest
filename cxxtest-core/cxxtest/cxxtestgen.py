...
def printVersion():
    ...
    sys.stdout.write( "This is CxxTest version 3.9.1.\n" )
    sys.exit(0)
...
exception_re = re.compile( r"\b(throw|try|catch|TSM?_ASSERT_THROWS|TSM?_ASSERT_THROWS_ANYTHING|TSM?_ASSERT_THROWS_NOTHING)\b" )
...
def writePreamble( output ):
    ...
    if gui:
        output.write( "#include <cxxtest/%s.h>\n" % gui )
    output.write( "\n" )
    output.write( "typedef const CxxTest::SuiteDescription *SuiteDescriptionPtr;\n" )
    output.write( "typedef const CxxTest::TestDescription *TestDescriptionPtr;\n" )
    output.write( "\n" )
    wrotePreamble = 1
