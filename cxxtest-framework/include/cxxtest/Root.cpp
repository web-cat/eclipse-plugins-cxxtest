...
#		ifdef CXXTEST_TRACE_STACK
    ...
    unsigned int          __cxxtest_stackTop;
    bool                  __cxxtest_handlingOverflow;
#		endif
...
#	endif
	...
	CXXTEST_EARLIEST_INIT(StringAppender __cxxtest_sigmsg);
	CXXTEST_EARLIEST_INIT(StringAppender __cxxtest_assertmsg);
...
#	ifdef CXXTEST_TRACE_STACK
	...
	static Dereferee::platform* __current_platform;
	...
	void __append_backtrace_plain(void** backtrace, bool filter, Appender& appender)
	{
		if(__current_platform == NULL)
		{
			Dereferee::option nullOption = { NULL, NULL };
		...
		if(backtrace != NULL)
		{
			...
			while(*bt)
			{
					...
					function, filename, &line_number))
				{
					...
					{
						appender.append_str("  - ");
						appender.append_str(function);

						if(line_number)
						{
							appender.append_str(" (");
							appender.append_str(filename);
							appender.append_str(":");
							appender.append_int(line_number);
							appender.append_str(")");
						}
...
//	void __cxxtest_sig_handler( int ) _CXXTEST_NO_INSTR;
	...
	void __cxxtest_sig_handler( int signum, siginfo_t* /*info*/, void* /*arg*/ )
	{
		...
		{
			msg = "unbounded recursion";
			CxxTest::__cxxtest_last_abort_was_overflow = true;
		}

		CxxTest::__cxxtest_sigmsg.clear();

		if ( !CxxTest::__cxxtest_assertmsg.empty() )
		{
			CxxTest::__cxxtest_sigmsg.append_str(CxxTest::__cxxtest_assertmsg.str());
			CxxTest::__cxxtest_assertmsg.clear();
		}
		else if ( CxxTest::__cxxtest_sigmsg.empty() )
		{
			CxxTest::__cxxtest_sigmsg.append_str(msg);
		}
		else
		{
			CxxTest::__cxxtest_sigmsg.append_str(msg);
//			CxxTest::__cxxtest_sigmsg.append_str(", maybe related to ");
//			CxxTest::__cxxtest_sigmsg.append_str();
//			CxxTest::__cxxtest_sigmsg = std::string(msg)
//				+ ", maybe related to " + CxxTest::__cxxtest_sigmsg;
		}
...
#	ifdef CXXTEST_TRACE_STACK
		...
		CxxTest::__cxxtest_sigmsg.append_str("\n");
		__append_backtrace_xml(backtrace, true, CxxTest::__cxxtest_sigmsg);
...
#	endif // _MSC_VER
    ...
    bool RealTestDescription::setUp()
    {
		...
		_TS_CATCH_SIGNAL({
			tracker().failedTest( file(), line(), __cxxtest_sigmsg.str() );
			ok = false;
		});
    ...
    bool RealWorldDescription::setUp()
    {
			...
			gf != 0; gf = gf->nextGlobalFixture() )
		{
			...
			_TS_CATCH_SIGNAL({
				doWarn( __FILE__, 1, __cxxtest_sigmsg.str() );
				ok = false;
			});
...
extern "C" {
...
#endif // __CYGWIN__
    {
		...
		CxxTest::__append_escaped_xml("assertion \"", appender);
		CxxTest::__append_escaped_xml(failedexpr, appender);
		CxxTest::__append_escaped_xml("\" failed: file \"", appender);
		CxxTest::__append_escaped_xml(file, appender);
		CxxTest::__append_escaped_xml("\", line ", appender);
		appender.append_int(line);
#else
		...
		CxxTest::__cxxtest_assertmsg.clear();
        CxxTest::__cxxtest_assertmsg.append_str(appender.str());
        abort();
    }
